<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            Загрузка товаров "В пути на WB"
        </h2>
    </x-slot>

    {{-- Убираем <script> для editableCell и всю логику SSE --}}
    {{-- Оставляем только meta csrf-token, если он нужен где-то еще на странице --}}
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                {{-- Форма загрузки файла (остается простой) --}}
                <form action="{{ route('import.in-transit-to-wb.process') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                        <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">
                            Загрузка файла "В пути на WB" (.xlsx)
                        </h3>
                        ...
                        @endif
                        @if (session('warning'))
                            <div class="mb-4 p-4 bg-yellow-100 dark:bg-yellow-900 border border-yellow-200 dark:border-yellow-700 text-yellow-700 dark:text-yellow-200 rounded">
                                {{ session('warning') }} <a href="#logs" class="underline">См. лог ниже.</a>
                            </div>
                        @endif
                        @if (session('error'))
            ...
            {{-- Блок с логами (без изменений, но данные будут появляться позже) --}}
            <div id="logs" class="mt-8">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">
                        История загрузок (ошибки и предупреждения)
                    </h3>
                    {{-- Кнопка обновить страницу --}}
                    <a href="{{ route('import.in-transit-to-wb.show') }}" class="text-sm text-blue-500 hover:underline">Обновить лог</a>
                </div>
                @if($logs->isEmpty())
                            ...
                            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                                <div class="p-4 border-b border-gray-200 dark:border-gray-700">
                                    <p class="text-sm font-semibold text-gray-700 dark:text-gray-300">
                                        ...
                                        {{-- Статус загрузки (очень упрощенно) --}}
                                        @php
                                            $hasErrors = $batchLogs->contains('status', 'error');
                                            $isFinished = $batchLogs->contains(fn($log)=>$log->message === "Обработка файла '{$log->originalFileName}' завершена." || str_contains($log->message, 'Критическая ошибка'));
                                            $isQueued = $batchLogs->contains('status', 'queued');
                                            $isProcessing = !$isFinished && !$isQueued && !$hasErrors; // Примерный статус
                                        @endphp
                                        @if($hasErrors) <span class="ml-2 px-2 py-0.5 text-xs font-semibold bg-red-100 text-red-800 rounded-full">Есть ошибки</span>
                                        @elseif($isQueued) <span class="ml-2 px-2 py-0.5 text-xs font-semibold bg-gray-100 text-gray-800 rounded-full">В очереди</span>
                                        ...
                                        @elseif($isFinished) <span class="ml-2 px-2 py-0.5 text-xs font-semibold bg-green-100 text-green-800 rounded-full">Завершено</span>
                                        @endif
                                    </p>
                                </div>
                                <div class="p-4 bg-gray-50 dark:bg-gray-800/50 max-h-60 overflow-y-auto text-xs font-mono">
                                    @foreach($batchLogs->sortBy('created_at') as $log) {{-- Сортируем по времени --}}
