    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <form action="{{ route('import.factory-order.process') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                        <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">
                            Загрузка файла "Заказ с завода" (.xlsx)
                        </h3>
                        <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
                            Система обработает **первый лист** файла. Строки будут сгруппированы по Артикулу (Колонка A). Баркоды (Колонка C) будут обновлены данными из их родительской строки.
                        </p>
            ...
            <div id="logs" class="mt-8">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">
                        История загрузок (Заказ с завода)
                    </h3>
                    <a href="{{ route('import.factory-order.show') }}" class="text-sm text-blue-500 hover:underline">Обновить лог</a>
                </div>
                @if($logs->isEmpty())
